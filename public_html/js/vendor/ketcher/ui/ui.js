...
//	ui.path = 'http://localhost:8082/'; //gpdc
//		jQuery.noConflict();
 //        jQuery.ajax({
//            type: "GET",
//            url: "http://localhost:8082/knocknock",
//            data: "",
//            async: false,
    ...
    if (mode == 'reaction_automap') {
        ui.showAutomapProperties({
            onOk: function(mode) {
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "http://localhost:8082/" + 'automap',
                    data: { moldata: moldata, mode: mode },
                    async: true,
                    success: function (data, textStatus) {

                        if (data.startsWith('Ok.')) {
                            ui.updateMolecule(ui.parseCTFile(data));
                        }
                    },
                    error: function (data, status, error) {
...
ui.loadMolecule = function (mol_string, force_layout, check_empty_line, paste)
{
    ...
    if (smiles.indexOf('\n') == -1)
    {
        ...
        jQuery.ajax({
            type: "POST",
            url: "http://localhost:8082/" + 'layout?smiles=' + encodeURIComponent(smiles),
            async: true,
            success: function (data, textStatus) {

                if (data.startsWith('Ok.'))
                    updateFunc.call(ui, ui.parseCTFile(data));
...
ui.dearomatizeMolecule = function (mol_string, aromatize)
{
    if (!ui.standalone)
    {
        ...
        jQuery.ajax({
            type: "POST",
            url: "http://localhost:8082/" + (aromatize ? 'aromatize' : 'dearomatize'),
            data: { moldata: mol_string },
            async: true,
            success: function (data, textStatus) {
